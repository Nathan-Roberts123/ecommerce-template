...
import CategoryListItem from "~/components/helpers/category-list-item";
import { CategoryItems } from "~/components/partials/header/category-list";
import { categoryItem } from "~/components/helpers/category-list-item";
...
export default function Drawer({ className, open, action }: Drawer) {
  return (
    <>
      <div
      ...
      >
        {open && (
          <div
            onClick={action}
            className="fixed left-0 top-0 z-40 h-screen w-full bg-black bg-opacity-40"
          ></div>
        )}
        <div
          className={`overflow-style-none fixed top-0 z-50 h-screen w-[280px] overflow-y-auto overflow-x-hidden bg-white transition-all  duration-300 ease-in-out ${
        ...
        >
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-5">
                <div className="favorite relative">
                  ...
                  <span className="absolute -right-2.5 -top-2.5 flex h-[18px] w-[18px] items-center justify-center rounded-full bg-qyellow text-[9px]">
                    1
                  </span>
                </div>
              </div>
          ...
          <div className="mt-5 w-full px-5">
            <div className="search-bar flex h-[34px]  w-full ">
              <div className="h-full flex-1 border border-r-0 border-[#E9E9E9] bg-white">
                <input
                  type="text"
                  className="foucus:ring-0 h-full w-full pl-2.5 text-xs placeholder:text-qgraytwo focus:outline-none "
                  placeholder="Search Product..."
                />
              </div>
              <div className="flex h-full w-[40px] items-center justify-center bg-qyellow">
                <span>
                  <svg
                    width="23"
                    height="23"
          ...
          <div className="mt-5 flex w-full items-center space-x-3 px-5">
            <span className="text-base font-semibold text-qblack">
              Categories
            </span>
          </div>
          ...
          <div className="category-item mt-5 w-full">
            <ul className="categories-list">
              {CategoryItems.map((item: categoryItem) => {
                return (
                  <CategoryListItem key={item.text} item={item} mobile={true} />
                );
              })}
            </ul>
          </div>
